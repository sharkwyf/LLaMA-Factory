...
class ScriptArguments:
    ...
    train_samples_per_iter: int = field(default=-1)
    test_samples_per_iter: int = field(default=-1)
    max_iters: int = field(default=-1)
...
async def main(script_args: ScriptArguments):
    ...
    console.print(f"Generating raw prompts, train: {len(train_dataset)}, test: {len(test_dataset)}")
    for dataset, split in [
        ...
        # Save
        output_dir = os.path.join(script_args.output_dir, "prompt")
        save_path = os.path.join(output_dir, f"{split}.json")
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        with open(save_path, "w") as f:
            json.dump(results, f, indent=4, ensure_ascii=False)
        console.print(f"Saving seed prompts {split} to {save_path}")

    # Generate
    for iter in range(1, script_args.max_iters + 1):
        console.print(f"Generating seed prompts for iter {iter}, train: {len(train_dataset)}, test: {len(test_dataset)}")
        for dataset, num_samples, split in [
            (train_dataset, script_args.train_samples_per_iter, "train"),
            (test_dataset, script_args.test_samples_per_iter, "test")
        ]:  
            if num_samples > 0:
                if split == "train":
                    selected_indices = np.random.choice(len(dataset), num_samples, replace=True)
                else:
                    selected_indices = np.arange(min(num_samples, len(dataset)))
            else:
                selected_indices = np.arange(len(dataset))
            truncated_dataset = dataset.select(selected_indices)
            ...
            # Save
            output_dir = os.path.join(script_args.output_dir, f"seed-{iter}")
            save_path = os.path.join(output_dir, f"{split}.json")
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            with open(save_path, "w") as f:
